...
router.get("/campgrounds",function(req,res){
    if(req.query.search){
        ...
        Campground.find({name:regex}, function (err, IdCampground) {
            if (err) {
                console.log(err);
            } else {
                res.render("campgrounds/index.ejs", { sites: IdCampground, userInfo: req.user });
            }
        });   
...
router.get("/campgrounds/:id",function(req,res){
    Campground.findById(req.params.id).populate("comments").exec(function(err,IdCampground){
        if(err){
            ...
            res.render("campgrounds/show.ejs",{IdCampground:IdCampground,moment:moment});
        }
    });
